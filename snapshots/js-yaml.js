...
function isInteger(object) {
	return Object.prototype.toString.call(object) === "[object Number]" && object % 1 === 0 && !common.c(object);
}
...
function readBlockScalar(state, nodeIndent) {
	var captureStart, folding, chomping = 1, didReadContent = false, detectedIndent = false, textIndent = nodeIndent, emptyLines = 0, atMoreIndented = false, tmp, ch;
	ch = state.input.charCodeAt(state.position);
	if (ch === 124) {
		folding = false;
	} else if (ch === 62) {
...
function readBlockMapping(state, nodeIndent, flowIndent) {
	...
	while (ch !== 0) {
		...
		if ((ch === 63 || ch === 58) && is_WS_OR_EOL(following)) {
			...
			if (!composeNode(state, flowIndent, 2, false, true)) {
