	...
	toRegExpSource(allowDot) {
		...
		if (!this.type) {
			...
			const src = this.#parts.map((p) => {
				const [re, , hasMagic, uflag] = typeof p === "string" ? AST.#parseGlob(p, this.#hasMagic, noEmpty) : p.toRegExpSource(allowDot);
	...
	*/
	...
	#backgroundFetch(k, index, options, context) {
		...
		const cb = (v, updateCache = false) => {
			...
			const proceed = options.ignoreFetchAbort || !!(options.allowStaleOnFetchAbort && v !== void 0);
			if (options.status) {
			...
			if (vl === p || ignoreAbort && updateCache && vl === void 0) {
				if (v === void 0) {
					if (bf.__staleWhileFetching !== void 0) {
		...
		const fetchFail = (er, proceed) => {
			...
			if (this.#valList[index] === p) {
				...
				const del = !noDelete || !proceed && bf.__staleWhileFetching === void 0;
				if (del) {
					this.#delete(k, "fetch");
		...
		const pcall = (res, rej) => {
			...
			if (fmp && fmp instanceof Promise) {
				fmp.then((v) => res(v === void 0 ? void 0 : v), rej);
			}
