...
function addOneShotEdges(ast, fg) {
    ...
    ast.attr.functions.forEach(function (fn) {
        const parent = fn.attr.parent;
        const childProp = fn.attr.childProp;

        if (childProp === 'callee' && parent &&
            (parent.type === 'CallExpression' || parent.type === 'NewExpression')) {
            // one-shot closure
            parent.attr.oneshot = true;
            for (let i = 0, nargs = parent.arguments.length; i < nargs; ++i) {
            ...
            for (let i = 0, nparms = fn.params.length; i <= nparms; ++i) {
    ...
    ast.attr.calls.forEach(function (call) {
        if (!call.attr.oneshot) {
            for (let i = 0, nargs = call.arguments.length; i <= nargs; ++i) {
