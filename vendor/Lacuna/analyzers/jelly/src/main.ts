...
program
    ...
    .option("-c , --callgraph-text <file>", "save reported call graph as a text file")
    ...
    if (options.dynamic) {
        ...
        if (options.typescriptLibraryUsage) {
            ...
            if (options.callgraph) {
                if(options.callgraphText) {
                    const file = options.callgraphText;
                    out.saveCallGraphAsText(file);
                    logger.info(`Call graph written to ${file}`);
                } else {
                    logger.info(`Option callgraphText not set`);
                }
            }
